...
- 2026-08-05 profile-account completeness: the source now distinguishes the editable single profile address from the unsupported multi-address book. Targeted analysis passed. The existing live server was not rebuilt during this non-mutating authenticated QA pass, so its capture remains evidence for the surrounding Profile capability layout rather than this latest source-only row.
