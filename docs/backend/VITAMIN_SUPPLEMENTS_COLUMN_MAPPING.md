...
| id/name/category and prices | metadata/provenance | no metadata column exists; `data_source=LEGACY_XLS_20260805`, `status=STAGING` |
...
The current schema has no product images, variants, separate prices/inventory, units, tax, manufacturer, or batch-provenance tables. No defaults are invented beyond staging classification and existing `products` scalar fields.
