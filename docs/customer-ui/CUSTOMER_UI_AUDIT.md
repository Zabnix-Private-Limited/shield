...
| `notifications` | Inline `_CustomerNotificationsView` | `GET /notifications`; read mutations are ownership-scoped | Extract and add read/filter states; customer ownership is enforced for individual and bulk read mutations |
