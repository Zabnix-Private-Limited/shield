...
| `activity` | `CustomerActivityScreen` | `GET /timeline/me` | Database-backed self timeline for membership, care, documents, wallet, and notifications; the endpoint takes no customer ID |
...
| `profile`, `settings` | Inline portal views | Customer profile API/session; alternative-contact APIs | Profile edits, persisted alternative-contact list/add, policy, support, feedback and sign-out are live. Contact reads/writes are self-scoped; customer preference, PIN, and care-sharing contracts remain unavailable; family/contact screens need extraction |
