...
| `appointments` | Inline `_CustomerAppointmentsView` | Appointment APIs | Customer owns only self-scoped list/read/cancel/reschedule actions, with explicit empty and retryable failure states; provider consultation and billing workflows are backend staff-only. Booking and visit-detail screens need feature extraction/visual redesign |
