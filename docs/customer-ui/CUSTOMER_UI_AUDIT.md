...
| `services` | Inline `_CustomerServicesView` | Live provider/profile APIs, authenticated `GET /customer/membership`, and `GET /customer/wellness-products` | Wellness catalogue is database-backed seeded demo data. Service eligibility now uses the authenticated membership bundle rather than profile-only data; laboratory/home-care and diet-plan catalogues remain unavailable until customer-safe contracts exist |
| `appointments` | Inline `_CustomerAppointmentsView` | Appointment APIs plus authenticated `GET /customer/membership` access check | Customer owns only self-scoped list/read/cancel/reschedule actions, with explicit empty and retryable failure states. The access gate now evaluates the authenticated profile and membership bundle together, so an active issued member is not locked out merely because the profile response omits membership data. Provider consultation and billing workflows are backend staff-only. Booking and visit-detail screens need feature extraction/visual redesign |
