...
| `profile`, `settings` | Inline portal views | Customer profile API/session; alternative-contact API | Profile edits, policy, support, feedback and sign-out are live. Alternative-contact writes are self-scoped; customer preference, PIN, and care-sharing contracts remain unavailable; family/contact screens need extraction |
