...
| `prescriptions` | `CustomerPrescriptionsScreen` | Customer-scoped documents, signed download, and prescription-review endpoint | Customer-owned prescription archive with loading, empty, and retryable generic failure states. Detail sheets open or download through authenticated signed URLs; review and processing-log reads enforce document ownership. Pharmacy linkage refinement remains. |
