...
| `documents` | `CustomerDocumentsScreen` | `GET /documents`, `GET /documents/:id`, `GET /documents/:id/download` | Customer-owned archive with loading, empty, and retryable generic failure states. Reads, signed downloads, and document-intelligence reads are ownership-checked; private storage paths are not returned to customers. Upload/view/share and visual QA remain. |
| `prescriptions` | `CustomerPrescriptionsScreen` | Customer-scoped documents and prescription-review endpoint | Customer-owned prescription archive with loading, empty, and retryable generic failure states. Review and processing-log reads enforce document ownership; viewer/pharmacy linkage refinement remains. |
