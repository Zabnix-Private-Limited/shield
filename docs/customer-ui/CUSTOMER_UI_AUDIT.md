...
| `documents` | `CustomerDocumentsScreen` | `GET /documents`, `GET /documents/:id`, `GET /documents/:id/download` | Customer-owned archive with loading, empty, and retryable generic failure states. Detail sheets open or download through authenticated signed URLs; reads and document-intelligence access are ownership-checked, and private storage paths are not returned. Upload/share and visual QA remain. |
