...
| Date/type query | `GET /wallets/:walletId/transactions` accepts `from`, `to`, and exact raw `type`, with customer wallet-ownership enforcement. It is not yet used by the customer bundle screen. | VERIFIED BACKEND CONTRACT — UI FOLLOW-UP |
