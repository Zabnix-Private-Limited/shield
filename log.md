...
- docs/customer-ui/CUSTOMER_UI_VISUAL_QA.md
\n\n## 31. Local web OTP launch configuration — 2026-08-06 00:52:00 IST\n\n- Propagated the existing debug-only ALLOW_LOCAL_WEB_PHONE_AUTH setting through the shared Flutter define helper and environment-aware launch/build scripts. The setting remains opt-in; no authentication bypass or credential change was added.\n\n### Frontend Files\n\n- frontend/scripts/flutter-env-defines.mjs\n- frontend/scripts/run-web-with-env.ps1\n- frontend/scripts/build-web-with-env.ps1\n- frontend/scripts/build-apk-with-env.ps1\n- frontend/scripts/build-apk-release-with-env.ps1\n\n### Verification\n\n- Flutter env define propagation check\n- flutter analyze auth repository and app config\n- git diff --check\n\n\n## 32. Customer account removal routing repair — 2026-08-06 00:58:00 IST\n\n- Corrected the Customer Account workspace so family removal uses the dependent API and contact removal uses the contact API.\n\n### Frontend Files\n\n- frontend/lib/features/customer/account/presentation/screens/customer_account_screen.dart\n\n### Verification\n\n- flutter analyze lib/features/customer/account/presentation/screens/customer_account_screen.dart\n- flutter test test/customer_portal_routes_test.dart\n- git diff --check\n\n\n## 33. Customer account state and profile links — 2026-08-06 01:10:00 IST\n\n- Moved customer account loading and refresh onto an auto-disposed Riverpod provider and wired Profile emergency-contact and preferred-pharmacy links to the real account workspace.\n\n### Frontend Files\n\n- frontend/lib/features/customer/account/presentation/screens/customer_account_screen.dart\n- frontend/lib/features/portal/presentation/screens/portal_shell.dart\n- frontend/test/customer_portal_routes_test.dart\n\n### Verification\n\n- flutter analyze customer account and portal files\n- flutter test test/customer_portal_routes_test.dart\n- git diff --check\n\n\n## 34. Typed-contact creation audit — 2026-08-06 01:16:00 IST\n\n- Added the missing account audit event for typed-contact creation while leaving the legacy alternative-contact endpoint behavior unchanged.\n\n### Backend Files\n\n- backend/src/customer/customer.service.ts\n\n### Verification\n\n- npm test -- --runInBand customer.service.spec.ts\n- npm run build\n- git diff --check\n\n\n## 35. Customer account form feedback — 2026-08-06 01:22:00 IST\n\n- Added visible validation and save-failure feedback for address, family-member, and typed-contact forms.\n\n### Frontend Files\n\n- frontend/lib/features/customer/account/presentation/screens/customer_account_screen.dart\n\n### Verification\n\n- dart format\n- flutter analyze lib/features/customer/account/presentation/screens/customer_account_screen.dart\n- git diff --check\n\n\n## 36. Customer account detail reads — 2026-08-06 01:30:00 IST\n\n- Added authenticated customer-scoped detail reads for addresses, family members, and typed contacts with non-disclosing ownership filters.\n\n### Backend Files\n\n- backend/src/customer/customer.service.ts\n- backend/src/customer/customer-account.controller.ts\n\n### Verification\n\n- npm test -- --runInBand customer-account.controller.spec.ts customer.service.spec.ts\n- npm run build\n- git diff --check\n
## 38. Wellness demo catalogue (2026-08-06 00:00:00 IST)
- Backend Files
  - Added customer-safe paginated wellness catalogue and detail contracts; imported LEGACY_XLS_20260805 rows are visible only when explicitly demo-enabled.
  - Added transactional publish, verify, and visibility-revert SQL plus product-service coverage.
- Frontend Files
...
## 52. Customer Web OTP send-lifecycle forensic audit (2026-08-14 00:10:00 IST)

- Source audit found exactly one Customer Firebase phone-send implementation: `CustomerAuthRepository.startPhoneVerification`. Its only callers are the explicit phone-entry Continue action and the explicit OTP-screen Resend action. No router redirect, OTP `initState`, post-frame callback, timer, Firebase auth listener, session probe, push bootstrap, or reCAPTCHA fallback invokes it.
- Single-flight: `_sendInFlight` returns the same operation to overlapping callers, so one in-flight request cannot create a second Firebase invocation. The countdown only changes `_remainingSeconds` and enables the button at zero; it never calls resend. OTP route init only starts/disposes that timer and, on non-web, probes already-authenticated native completion; it does not send.
- Confirmation ownership: the singleton repository holds one web `ConfirmationResult`; it is set only after a successful `signInWithPhoneNumber` and survives route navigation. Failed resend leaves it untouched; a successful explicit resend replaces it. Overlapping completions cannot overwrite it because a later request cannot start until `_sendInFlight` clears.
- Recent history: `ac268a7` introduced the web `signInWithPhoneNumber` path and reCAPTCHA warmup. The warmup was removed later; it did not call phone send. `d98541c` added single-flight/cooldown and suppressed the web auto-session probe; no history change introduced automatic resend or application-level Enterprise-to-v2 retry.
- Diagnostics: repository now logs `OTP_SEND_START/SUCCESS/FAILURE` with local attempt ID, trigger, confirmation generation, and masked phone suffix only. It never logs OTP, full phone, sessionInfo, reCAPTCHA, Firebase ID token, or refresh token.
- Verification: focused `flutter analyze` passed; focused state/error/logo suite passed (14); `git diff --check` passed. Physical SMS delivery remains an external Firebase/carrier question and was not claimed fixed.
## 53. Customer OTP runtime evidence classification (2026-08-14 11:35:00 IST)

- Production evidence: Firebase `accounts:sendVerificationCode` returned HTTP 200 and Firebase Usage counted SMS as sent. A later `accounts:signInWithPhoneNumber` 400 `INVALID_CODE` followed manual entry of an incorrect code, so it is confirmation failure, not send failure. Current UI maps `invalid-verification-code` to a specific invalid-OTP message rather than network failure.
- Deployed diagnostic identity is not proven: local HEAD `700d536` contains `OTP_SEND_*`, but those lines are not visible in the supplied production console. This is consistent with an older deployed frontend or filtered/suppressed Flutter console output; it cannot establish invocation count.
- `/auth/me` 401 is not an unauthenticated OTP bootstrap send. Source calls it only during restoration when secure storage has an access token. `/auth/*` 401 does not enter the refresh/retry interceptor; session initialization catches it and clears local storage. It does not start phone verification or produce an OTP customer error.
- Firebase owns Enterprise-to-v2 reCAPTCHA fallback. The frontend has no fallback handler/retry that calls `startPhoneVerification` after Firebase fallback.
- Runtime blocker: one controlled action cannot yet be correlated to one application attempt and one network POST until the deployed revision/console logging is identified. No Firebase security or OTP behavior changed in this verification slice.
## 54. Customer OTP runtime closure — number-specific delivery issue (2026-08-14 11:45:00 IST)

- New production UAT authenticated a different real customer number through the same deployed SHIELD web flow and reached the Customer dashboard. This proves the application OTP send/confirm path, Firebase Phone Auth project, deployed domain/configuration, India delivery path, billing setup, and Firebase-owned reCAPTCHA fallback are functional end to end.
