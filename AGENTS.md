...
- Wallet balances are ledger-based and must remain separated across `CASH`, `REWARD_POINTS`, and `SHIELD_BENEFIT` entries. `SHIELD_BENEFIT` is separately tracked from cash and reward points, but its available balance and transactions are customer-visible.
- **PRIVILEGE MEMBER WALLET RECHARGE RULE**: Counter wallet recharges and 10% extra promotional credit require both an active, unexpired SHIELD Membership and a SHIELD Privilege Card in `ISSUED` or `ACTIVE` status; `customer_code` is never proof of recharge eligibility. The customer wallet must be active when one already exists. Minimum recharge is ₹10,000 in exact multiples of ₹10,000. Non-member app users can only purchase wellness products directly from customer-facing app interfaces and cannot hold wallet balances. Recharge itself is funding, so sufficient cash balance is enforced only at wallet spend/redemption time.
- `SHIELD_BENEFIT` is company-funded promotional credit. It must be shown separately to customers as an available promotional-benefit balance and transaction history, without merging it into cash or reward points.
