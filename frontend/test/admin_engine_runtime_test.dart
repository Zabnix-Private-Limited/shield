...
void main() {
  group('AdminPlatformRuntime', () {
    test(
      'bootstraps workspace, navigation, capability, and schema registries from the shared catalog',
      () {
        ...
        expect(
          runtime.registrationFor('settings')?.workspace.title,
          'Settings',
        );
        expect(
        ...
        expect(
          runtime.capabilityRegistry.supports('settings', 'forms'),
          isTrue,
        );
        expect(
          runtime.capabilityRegistry.supports('platform', 'tables'),
          isTrue,
        );
        expect(runtime.schemaFor('audit')?.defaultViewId, 'table');
      },
    );

    testWidgets(
      'AdminPortalWorkspace renders through an injected runtime registration',
      (tester) async {
        final runtime = AdminPlatformRuntime.bootstrap(
          registrations: [
  ...
  group('Admin navigation platform', () {
    test(
      'resolves navigation definitions by route and keeps active workspace in sync',
      () {
        ...
        expect(controller.activeNavigation?.breadcrumbs, [
          'Super Admin',
          'Customers',
        ]);
      },
  ...
  group('AdminWorkspaceController', () {
    test(
      'loads workspace definition, schema, permissions, data, and emits metadata-rich events',
      () async {
          ...
          views: [
            AdminViewDefinition(
              ...
              table: AdminTableDefinition(
                ...
                sorting: const [
                  AdminSortDefinition(key: 'name', label: 'Name'),
                ],
                actions: const [
        ...
        expect(controller.state.status, AdminWorkspaceStatus.ready);
        expect(
          controller.snapshot?.navigation.route,
          '/portal/super-admin/customers',
        );
        expect(controller.snapshot?.schema.defaultViewId, 'table');
        expect(
    ...
    test(
      'moves into permission denied state when workspace access is blocked',
      () async {
        final workspaceRegistry = AdminWorkspaceRegistry()
          ..register(
            const AdminWorkspaceDefinition(
  ...
  group('Admin action pipeline', () {
    test(
      'runs middleware, dispatches repositories, and publishes correlated action events',
      () async {
  ...
  group('Customer repository contracts', () {
    test(
      'supports in-memory customer mutations behind the repository interface',
      () async {
...
Widget _runtimeTestBuilder(
  ...
  return Text(
    snapshot.workspace.title == 'Runtime Test'
